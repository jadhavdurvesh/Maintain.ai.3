...
def register(payload: RegisterIn, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(org)
    user = models.User(username=payload.username, full_name=payload.full_name, email=payload.email, password_hash=hash_password(payload.password), organization_id=org.id, role=models.UserRole.admin, active=True)
    db.add(user)
    db.commit()
    db.refresh(user)
    ...
    return AuthOut(access_token=token, user_id=user.id, username=user.username, organization_id=org.id, organization_name=org.name, role=user.role.value)
...
def login(payload: LoginIn, db: Session = Depends(get_db)):
    ...
    if not user or not user.active or not user.password_hash or not verify_password(payload.password, user.password_hash):
        raise HTTPException(401, "incorrect email, password, or inactive account")
    org = db.get(models.Organization, user.organization_id)
    if not org:
        raise HTTPException(401, "account organization not found")
    token = create_access_token(user.id, user.organization_id)
    return AuthOut(access_token=token, user_id=user.id, username=user.username, organization_id=user.organization_id, organization_name=org.name, role=user.role.value)
...
def me(current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {"user_id": current.id, "username": current.username, "role": current.role, "organization_id": current.organization_id, "organization_name": org.name if org else None}
