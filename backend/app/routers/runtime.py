...
# Runtime is intentionally separate from Machine.status: status describes health,
# while runtime_state describes whether the asset is actually consuming hours.
metadata = MetaData()
...
def _ensure_table(db: Session):
    runtime_states.create(bind=db.get_bind(), checkfirst=True)


def _is_runtime_manager(current: CurrentUser) -> bool:
    return current.role in {
        models.UserRole.admin.value,
        models.UserRole.technician.value,
    }
...
def _row(db: Session, machine):
    result = db.execute(
        runtime_states.select().where(runtime_states.c.machine_id == machine.id)
    ).mappings().first()
    if result:
        return result
...
def _hours(machine, row, now=None):
    ...
    return base + elapsed / 3600.0


def _snapshot(machine, row):
    now = datetime.utcnow()
    hours = _hours(machine, row, now)
    return {
        "machine_id": machine.id,
        "state": row["state"],
        "operating_hours": round(hours, 4),
...
def set_runtime_state(
    ...
    # Settle accumulated hours before changing state so idle/maintenance/fault
    # never consume operating time and a later run continues from the exact total.
    settled_hours = _hours(machine, row, now)
    previous = row["state"]
    started_at = now if state == _ACTIVE_STATE else None
    base_hours = settled_hours

    db.execute(
        runtime_states.update()
        .where(runtime_states.c.machine_id == machine.id)
        .values(
            state=state,
            started_at=started_at,
            base_operating_hours=base_hours,
            updated_at=now,
        )
    )

    # Keep the canonical machine counter synchronized whenever a transition
    # stops the clock. While running, GET /runtime computes the live value.
