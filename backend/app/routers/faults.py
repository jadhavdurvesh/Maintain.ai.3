...
def list_faults(machine_id: int | None = None, unresolved_only: bool = False, current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    q = db.query(models.FaultRecord).join(models.Machine).filter(models.Machine.organization_id == current.organization_id)
...
def create_fault(payload: schemas.FaultRecordIn, current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(fault)
    audit.log_event(db, "fault", fault.id, "reported", f"Fault reported for {machine.name}: {fault.description}")
...
def resolve_fault(fault_id: int, payload: schemas.FaultResolveIn, current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(fault)
    audit.log_event(db, "fault", fault.id, "resolved", f"Fault resolved for {fault.machine.name}: {fault.resolution}")
    return fault
...
def create_work_order_from_fault(fault_id: int, payload: schemas.WorkOrderIn | None = None, current: CurrentUser = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Explicit fault_id makes the relationship permanent and prevents the same
    # fault from generating multiple active work orders.
    existing = (
    ...
    if existing is None:
        # Backward compatibility for work orders created before fault_id existed.
        existing = (
            db.query(models.WorkOrder)
            .filter(
        ...
        if existing is not None and existing.fault_id is None:
            existing.fault_id = fault.id
            db.commit()
            db.refresh(existing)
    ...
    data["recommended_actions"] = data.get("recommended_actions") or (f"Investigate fault: {fault.symptoms}" if fault.symptoms else "Investigate reported fault and confirm root cause.")
    ...
    db.commit()
    db.refresh(wo)
    audit.log_event(db, "fault", fault.id, "work_order_created", f"Work order #{wo.id} created from fault #{fault.id}" + (f" and assigned to {wo.assigned_to}" if wo.assigned_to else ""), performed_by=current.username)
    return wo
