...
export default function SpareParts() {
  ...
  useEffect(() => { load() }, [])
  const create = async (e) => { e.preventDefault(); await api.post('/api/spare-parts', { ...form, quantity: Number(form.quantity), minimum_stock: Number(form.minimum_stock) }); setForm({ name: '', part_number: '', quantity: 0, minimum_stock: 1, compatible_machine_categories: '' }); setShowForm(false); load() }
